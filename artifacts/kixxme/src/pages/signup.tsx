    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center p-4 bg-background">
      <div className="w-full max-w-sm space-y-8">
        <div className="text-center">
          <h1 className="text-6xl font-display uppercase tracking-tight" data-testid="heading-signup">KIXXME</h1>
          <p className="text-muted-foreground font-sans mt-2" data-testid="text-signup-sub">Claim your locker.</p>
        </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-display text-xl uppercase">Username</FormLabel>
                    <FormControl>
                      <Input placeholder="athlete123" {...field} className="border-2 border-border rounded-none focus-visible:ring-primary focus-visible:ring-offset-0 focus-visible:border-primary text-lg p-6 font-sans" data-testid="input-username" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
        ...
        <p className="text-center text-muted-foreground font-sans" data-testid="text-login-prompt">
          Already have a locker? <Link href="/login" className="text-primary font-bold hover:underline" data-testid="link-login">Enter here.</Link>
        </p>
      </div>
    </div>
  );
