...
- [Supabase shared-client session pollution](kixxme-supabase-client-session-pollution.md) — never run signIn/signUp/refresh/setSession on a service-role/data client; it demotes all `.from()` to that user's RLS process-wide. Use a dedicated anon auth client.
...
- [PWA & mobile packaging](kixxme-pwa-mobile.md) — prod-only SW must never touch `/api` or cross-origin (Supabase); `assetlinks.json` is served file-first despite the SPA rewrite; Play fingerprint = Play App Signing key, not the local keystore.
