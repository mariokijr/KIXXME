...
**Active prod config (verified June 2026):** `EMAIL_FROM="KixxMe <support@kixxme.com>"` — kixxme.com domain is **verified in Resend** (SPF/DKIM done). All transactional email flows through Resend from `support@kixxme.com`; `supportkixxme@gmail.com` is Reply-To only. Gmail connector is silent fallback only. Resend quota-exhaustion cache is 10 min (was midnight UTC) so the system auto-retries after plan upgrades. The three prod secrets: `RESEND_API_KEY`, `EMAIL_FROM`, `APP_BASE_URL=https://kixxme.com`. Gmail fallback MUST keep its From = the connected `supportkixxme@gmail.com` (Gmail can't send as kixxme.com).
