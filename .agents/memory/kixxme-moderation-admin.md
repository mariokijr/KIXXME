...
## Admin summary "all zeros" is often correct, not a bug
- The dashboard counters (open reports / flags / pending verifications / suspended / banned / removed / open tickets) count only OPEN *qualifying* rows. **Warnings have no counter** (they change no account state — only history + email), an **approved or rejected verification drops `pending` to 0**, and flags need REPORT_FLAG_THRESHOLD (3) distinct reporters. An admin who only warned a couple users and approved their own verification will correctly see all zeros.
- **Why:** the panel reads real data; "everything is 0" usually means the dev DB genuinely has no qualifying rows, not that the summary query is broken. Confirm the underlying rows (psql counts) before chasing a phantom summary bug.
- **Self-report 400 trap:** `POST /reports` rejects reporting yourself ("No puedes reportarte a ti mismo") and the dialog only shows a destructive toast — an admin self-testing with a single account produces zero report rows even though the flow works end-to-end. Test reports from a *second* account and watch for the green "Reporte enviado" toast.
